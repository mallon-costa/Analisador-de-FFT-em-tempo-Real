...
class Spectrum_Visualizer:
    ...
    def __init__(self, ear):
        ...
        self.tag_every_n_bins = max(1,round(5 * (self.ear.n_frequency_bins / 51))) #Ocasionalmente exibe tags Hz no eixo x
        ...
        #Parâmetros de inicialização fixos:
    ...
    def toggle_history_mode(self):
        ...
        if self.plot_audio_history:
            self.bg_color           = 10    #Cor de fundo
            self.decay_speed        = 0.10  #Decaimento vertical de barras lentas
    ...
    def start(self):
        print("Iniciando visualizador de espectro...")
        pygame.init()
        ...
        pygame.display.set_caption('Analizador de Espectro -- (FFT-Pico: %05d Hz)' %self.ear.strongest_frequency)
        ...
        self.history_button  = Button(text="Alternar o modo 2D/3D", right=self.WIDTH, top=0, width=round(0.12*self.WIDTH), height=self.button_height)
        self.slow_bar_button = Button(text="Alternar barras lentas", right=self.WIDTH, top=self.history_button.height, width=round(0.12*self.WIDTH), height=self.button_height)

    def stop(self):
        print("Parando o visualizador de espectro...")
        del self.fps_font
        del self.bin_font
        del self.screen
    ...
    def toggle_display(self):
        ...
        #Esta função pode ser acionada para ligar/desligar o display
