...
#Algumas funções úteis para o acionamento de outras classes
...
def gaussian_kernel1d(sigma, truncate=2.0):
    sigma = float(sigma)
    sigma2 = sigma * sigma
    #tornar o raio do filtro igual a truncar os desvios padrão
...
def get_smoothing_filter(FFT_window_size_ms, filter_length_ms, verbose = 0):
    ...
    filter_sigma = buffer_length / 3  #A rapidez com que a influência de suavização cai sobre o comprimento do buffer
    ...
    if verbose:
        ...
        print("O buffer de suavização contém %d janela FFT (sigma: %.3f) --> min_contribuição: %.3f%%" %(buffer_length, filter_sigma, min_fraction))
...
class numpy_data_buffer:
    """
    Um buffer FIFO rápido e circular em numpy com interações mínimas de memória usando uma matriz de ponteiros de índice
    """
...
class Button:
    def __init__(self, text="", right=10, top=30, width=None, height=20):
        ...
        if width == None:
            ...
            self.width_type = "texto"
        else:
            self.width = width
            self.width_type = "usuario"
